...
# Usage: python3 scripts/loss_plot.py train-log-002.txt [損失グラフのテーブルの幅] [損失グラフのタイトル]
...
def get_loss_list(path, plot_loss_list):
    epoch_data_list = []
    epoch_list = [0]
    step_list = [0]
    game_list = [0]
    ...
    for line in lines:
        ...
        if line.startswith('loss'):
            ...
            for e in elms[2:]:
                name, loss = e.split(':')
                if not plot_loss_list or name in plot_loss_list:
                    loss = float(loss)
...
# plot_loss_list = sys.argv[3].split(',') if len(sys.argv) > 3 else []
plot_loss_list = sys.argv[4].split(',') if len(sys.argv) > 4 else []
clipped_epoch_list, clipped_step_list, clipped_game_list, averaged_loss_lists, start_epoch = get_loss_list(sys.argv[1], plot_loss_list)
...
for i, (k, v) in enumerate(averaged_loss_lists.items()):
    if fig_num == 1:
        ...
        ax = fig.add_subplot((fig_num - 1) // width + 1, width, i + 1)
    start = start_epoch
