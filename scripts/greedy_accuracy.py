# Usage: python3 scripts/greedy_accuracy.py [任意のlogの数] [任意のタイトル名] [ディレクトリのパス]
# # Usage: python3 scripts/greedy_accuracy.py 0 greedy_accuracy 202301010101
...
def get_reward_list(path):
    ...
    for line in lines:
        ...
        if line.startswith('loss'):
            ...
            for e in elms[2:]:
                name, loss = e.split(':')
                if name == "greedy_select" or name=="greedy_nn" or name=="greedy_reg":
                    loss = float(loss)
...
for i in logs[:a]:
    ...
    if 'greedy_nn' in averaged_loss_lists:
        nn_mean += averaged_loss_lists['greedy_nn']
        nn_flag = True
    if 'greedy_reg' in averaged_loss_lists:
        reg_mean += averaged_loss_lists['greedy_reg']
        reg_flag = True
    if 'greedy_select' in averaged_loss_lists:
        greedy_select_mean += averaged_loss_lists['greedy_select']

nn_mean =  nn_mean / a #平均を取る
reg_mean = reg_mean / a
greedy_select_mean = greedy_select_mean / a
...
for opponent in opponents:
    ...
    if opponent == 'greedy_nn':
        np.savetxt(path + 'greedy_nn.csv', [loss_list[start:]], delimiter=',', fmt='%.5f')
        np.savetxt(path + 'en_episodes.csv', [clipped_game_list[start:]], delimiter=',', fmt='%d')
    elif opponent == 'greedy_reg':
        np.savetxt(path + 'greedy_reg.csv', [loss_list[start:]], delimiter=',', fmt='%.5f')
        np.savetxt(path + 'en_episodes.csv', [clipped_game_list[start:]], delimiter=',', fmt='%d')
    elif opponent == 'greedy_select':
        np.savetxt(path + 'greedy_select.csv', [loss_list[start:]], delimiter=',', fmt='%.5f')
        np.savetxt(path + 'en_episodes.csv', [clipped_game_list[start:]], delimiter=',', fmt='%d')
    else:
