...
def get_wp_list(path):
    ...
    kn = kernel(n)
    #print(kn)
    averaged_reward_lists = {} #平均勝率のリスト
    start_epoch = {}
    for opponent in opponents:
        ...
        averaged_reward_list = np.convolve(reward_list, kn, mode='valid') + 1e-6
    ...
    return config_list, clipped_epoch_list, clipped_step_list, clipped_game_list, averaged_reward_lists, start_epoch
...
for fname in files_dir: #ディレクトリの中のログディレクトの個数for文を回す
    ...
    mean = 0 #平均を仮で入れる
    for j in logs[:a]:
        config_list, clipped_epoch_list, clipped_step_list, clipped_game_list, averaged_reward_lists, start_epoch = get_wp_list(j) #get_wp_listでlogデータを抽出
        print(j,averaged_reward_lists)
    ...
    for opponent in opponents:
        ...
        ax.plot(clipped_game_list[start:], reward_list[start:], label=label_name)
